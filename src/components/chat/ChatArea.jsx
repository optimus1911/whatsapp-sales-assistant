...
import { IoHappyOutline, IoAttachOutline, IoSend, IoMicOutline, IoCheckmarkDone, IoCheckmark, IoSparklesOutline } from 'react-icons/io5';
...
export default function ChatArea() {
  ...
  // Handle manual message post submission
  const handleSend = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        // Append sent message in CRM Context
  ...
  if (!activeCustomerId) {
    return (
      <div className="flex-1 flex items-center justify-center bg-whatsapp-dark text-whatsapp-gray text-sm select-none">
          ...
          <div className="flex items-center justify-between px-4 py-2 bg-whatsapp-panel border-b border-whatsapp-border/20 z-10">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all duration-200 ${
                intelPanelOpen
                  ? "bg-whatsapp-teal/20 border-whatsapp-green/30 text-whatsapp-green"
                  : "bg-whatsapp-input border-whatsapp-border/20 text-whatsapp-gray hover:text-white"
              }`}
            >
              <IoSparklesOutline className={intelPanelOpen ? "animate-pulse" : ""} />
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-2 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-whatsapp-dark/95 to-whatsapp-dark/99 relative">
          ...
          {isThinking && (
            <div className="flex justify-start w-full animate-pulse select-none">
              <div className="bg-whatsapp-incoming text-white rounded-lg rounded-tl-none px-3.5 py-2.5 shadow-sm max-w-[65%] flex flex-col gap-1.5 border border-whatsapp-border/10">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-whatsapp-green font-semibold uppercase tracking-wider font-mono">AI Thinking</span>
                </div>
                <span className="text-xs text-whatsapp-gray font-medium transition-all duration-300">
